...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive):
        ...
        while current_place != hive:
            if current_place.bees and  throw_distance <= self.max_range and throw_distance >= self.min_range:
...
# END Problem 13
    ...
    def __init__(self):
        # BEGIN Problem 13
        if QueenAnt.first_queen_exist:
            self.impostor_status = True
        Ant.__init__(self)
    ...
    def action(self, colony):
        ...
        """
        # BEGIN Problem 13
        if self.impostor_status:
            Insect.reduce_armor(self,self.armor)
        else:
            '''
            for follower in self.ant_followers_list:
                follower.damage = follower.damage // 2

            self.ant_followers_list = []
            '''
            ThrowerAnt.action(self,colony)
            current_exit = self.place.exit
            while current_exit:
                follower = current_exit.ant
                if follower:
                    if not follower in self.ant_followers_list:
                        follower.damage = follower.damage * 2
                        self.ant_followers_list.append(follower)
                    if follower.container:
                        inside_follower = follower.ant
                        if inside_follower and not inside_follower in self.ant_followers_list:
                            inside_follower.damage = inside_follower.damage * 2
                            self.ant_followers_list.append(inside_follower)
